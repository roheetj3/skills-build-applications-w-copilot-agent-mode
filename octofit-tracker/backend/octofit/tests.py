...
class ActivityModelTest(TestCase):
    def test_create_activity(self):
        ...
        activity = Activity.objects.create(user=user, activity_type='run', duration=30, date='2025-04-26')
        self.assertEqual(activity.activity_type, 'run')

class WorkoutModelTest(TestCase):
    def test_create_workout(self):
        user = User.objects.create(email='workout@example.com', name='Workout User', password='testpass')
        workout = Workout.objects.create(user=user, description='Pushups', date='2025-04-26')
